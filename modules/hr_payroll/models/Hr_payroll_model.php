...
/* Chứ năng đồng bộ dữ liệu từ HR records module ( thông tin liên quan đến lương của nhân viên trong hợp đồng), thiếu trường hợp khi đồng bọ nếu nhân viên đã tồn tại thì chỉ cập nhật, không tạo mới, trùng cột khi cập nhật*/
/* thêm cột, xóa cột khi cài đặt có thay đổi, 
 TH1: Đồng bộ lần đầu khi chưa có dữ liệu
 TH2: Đồng bộ sau khi có dữ liệu: có thể một số dữ liệu master được thây đổi ( thêm, xóa, chỉnh sửa)
 */
	/**
...
/*TO DO: XỬ LÝ CẬP NHẬT, cập nhật thông tin nhân viên sau khi người dùng thực hiện thay đổi trên handsome table, dùng cho 2 trường hợp: có tích hợp HR records module, không tích hợp Hr records module.  đang xử lý!!!*/
	...
	public function add_payslip_templates_detail_first($id, $update= false, $old_column_formular=[])
	{
		...
		if($payslip_templates){
			...
			$payslip_template_data_update[] = hrp_payslip_replace_string($payslip_template_data);
		...
		foreach ($payslip_detail as $key => $value) {
			...
			foreach ($value as $payroll_key => $payroll_value) {
				if($payroll_key == 'payment_run_date'){
					$payslip_detail[$key][$payroll_key] = DateTime::createFromFormat('d/m/Y',$payroll_value)->format('Y-m-d');
				}
			...
			if(count($array_staff_ids) > 0){
				...
				for ($i=0; $i < count($payslip_templates); $i++) { 
					if($payslip_templates[$i]['staff_employees'] != '' || $payslip_templates[$i]['staff_employees'] != null){
						...
						$get_staffid_by_payslip_template = $this->payslip_template_get_staffid($payslip_templates[$i]['department_id'], $payslip_templates[$i]['role_employees'], $payslip_templates[$i]['staff_employees']);
