...
class hr_payroll extends AdminController {
	...
	{
		if (!has_permission('hrp_attendance', '', 'view') && !is_admin()) {
	...
	public function attendance_filter()
	{
		...
		$index_data_object=0;
		if($newquerystring != ''){
			$staffs = $this->hr_payroll_model->getStaff('', $newquerystring); 
	...
	public function import_attendance_excel()
	{
		...
		{
			if (!has_permission('hrp_deduction', '', 'view') && !is_admin()) {
		...
		{
			if (!has_permission('hrp_commission', '', 'view') && !is_admin()) {
	...
	{
		if (!has_permission('hrp_income_tax', '', 'view') && !is_admin()) {
	...
	{
		if (!has_permission('hrp_insurrance', '', 'view') && !is_admin()) {
	...
	{
		if (!has_permission('hrp_payslip', '', 'view')  && !is_admin()) {
	...
	{   
		if (!has_permission('hrp_payslip_template', '', 'view') && !is_admin()) {
	...
	public function view_payslip_templates_detail($id = ""){
		...
		if($this->input->post()){
			$data = $this->input->post();
				...
				if (!is_admin() && !has_permission('hrp_payslip_template', '', 'edit') && !has_permission('hrp_payslip_template', '', 'add')) {
					access_denied('hr_payroll');
				}
				$id = $data['id'];
				unset($data['id']);
	...
	public function view_payslip_detail($id = ""){
		...
		if($this->input->post()){
			$data = $this->input->post();
				...
				if (!is_admin() && !has_permission('hrp_payslip', '', 'edit') && !has_permission('hrp_payslip', '', 'add')) {
					access_denied('hrp_payslip');
				}
				$id = $data['id'];
				unset($data['id']);
	...
	{
		if (!has_permission('hrp_bonus_kpi', '', 'view') && !is_admin()) {
