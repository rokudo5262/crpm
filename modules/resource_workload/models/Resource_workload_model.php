...
class Resource_workload_model extends App_Model
{
    ...
    public function get_data_workload($data_fill, $only_data = false)
    {
        ...
        foreach ($staffsTasks as $key => $value) {
            if(strtotime($task_from_date) > strtotime($value['startdate'])){
                $task_from_date = $value['startdate'];
            }

            if(strtotime($task_to_date) < strtotime($value['duedate'])){
        ...
        $standard_workload = $this->get_standard_workload_by_list_staff($list_staffids, $task_from_date, $task_to_date);
        ...
        foreach ($data_array as $key => $value) {
            ...
            if (!isset($data[$staffid]['capacity'])) {
                ...
                if ($only_data == false) {
                    $data_tooltip_2[$staffid] = $this->create_array_data_tooltip_workload($value['staffid'], $from_date, $to_date, []);
                }
            }
            ...
            foreach ($val as $key => $value) {
                if ($only_data == false) {
                    if (!(strpos($key, '_e') === false) || !(strpos($key, '_s') === false)) {
                        $k                 = explode('_', $key);
                        if ($standard_workload[$val['staff_id']][$k[2] . '-' . $k[1] . '-' . $k[0]] < $value) {
                            $overload[$key] = 1;
                        }
                    }
                }
    ...
    public function get_standard_workload_by_list_staff($list_staff, $from_date, $to_date)
    {
        ...
        if(get_option('integrated_timesheet_leave') == 1 && $this->get_status_modules_all('timesheets')){
            ...
            $this->db->where('date_work >= "'. $from_date.'"');
            $this->db->where('date_work <= "'. $to_date.'"');
            $this->db->where('relate_type', 'leave');

            $timesheets = $this->db->get(db_prefix() . 'timesheets_timesheet')->result_array();
            foreach ($timesheets as $key => $value) {
                if(isset($data_timesheets[$value['staff_id']][$value['date_work']])){
                    $data_timesheets[$value['staff_id']][$value['date_work']] += $value['value'];
                }else{
        ...
        while (strtotime($from_date) <= strtotime($to_date)) {
            foreach ($standard_workload as $key => $value) {
                $leave = 0;
                if(isset($data_timesheets[$value['staffid']][$from_date])){
                    $leave = $data_timesheets[$value['staffid']][$from_date];
                }
                switch (date('N', strtotime($from_date))) {
                    case 1:
                        $working_days[$value['staffid']][$from_date] = $value['monday'] - $leave;
                        break;
                    case 2:
                        $working_days[$value['staffid']][$from_date] = $value['tuesday'] - $leave;
                        break;
                    case 3:
                        $working_days[$value['staffid']][$from_date] = $value['wednesday'] - $leave;
                        break;
                    case 4:
                        $working_days[$value['staffid']][$from_date] = $value['thursday'] - $leave;
                        break;
                    case 5:
                        $working_days[$value['staffid']][$from_date] = $value['friday'] - $leave;
                        break;
                    case 6:
                        $working_days[$value['staffid']][$from_date] = $value['saturday'] - $leave;
                        break;
                    case 7:
                        $working_days[$value['staffid']][$from_date] = $value['sunday'] - $leave;
                        break;
            ...
            foreach ($list_staff as $staffid) {
                if(!isset($working_days[$staffid][$from_date])){
                    ...
                    if(isset($data_timesheets[$staffid][$from_date])){
                        $leave = $data_timesheets[$staffid][$from_date];
                    }
                    $working_days[$staffid][$from_date] = $standard_workload_default - $leave;
                }
            }
            $from_date = date('Y-m-d', strtotime('+1 day', strtotime($from_date)));
        }
    ...
    public function get_data_capacity($data_fill){
        ...
        $data_return['total']['total_capacity'] = $total.' '._l('hours');
        return $data_return;
    }
