  ...
  gantt = new Gantt("#timeline", tasks, {
      custom_popup_html: function(task) {
        ...
        if(task.total_time != undefined){
          total_time = '<p class="details_title"> Total time: '+task.total_time.toFixed(2)+'</p>';
        }else{
          total_time = ''
        }
        var estimate_hour = 0;
        if(task.estimate_hour != undefined){
          estimate_hour = '<p class="details_title"> Estimate hour: '+task.estimate_hour.toFixed(2)+'</p>';
        }else{
          estimate_hour = ''
        }
        return `
